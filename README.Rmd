...
6 - Exportar o resultado do Scraping para um repositório público (ex: Docker) para que qualquer pessoa possa baixar os dados.

7 - Desenvolver um ChatBot no Telegram para sinalizar ao mantenedor do Web Scraping quando ele inicia e termina, ou quando ocorre um erro que interrompe a execução.
...
2º - Agregar possíveis insights aplicados no pacote RCrawler
...
!!! Bug de recomeço da execução. Se estiver no mesmo diretório dará problema. Já se estiver um nível acima, funciona normalmente.
Quado se estar dentro do diretório do Web Scraping
[1] "O diretório Raiz foi definido com sucesso!"
Error in (function (classes, fdef, mtable)  : 
  unable to find an inherited method for function ‘dbWriteTable’ for signature ‘"SQLiteConnection", "character"’
  
  
!!! Verificar esse erro no arquivo html após a Prefeitura Municipal de Salinas (Talvez seja Salvador)
    Talvez seja a necessidade de apagar a lixeira do Linux
Error in node_write_file(x$node, file, options = options, encoding = encoding) : 
  Error closing file
In addition: Warning messages:
1: In node_write_file(x$node, file, options = options, encoding = encoding) :
  No space left on devic [1528]
2: In node_write_file(x$node, file, options = options, encoding = encoding) :
  write erro [1546]